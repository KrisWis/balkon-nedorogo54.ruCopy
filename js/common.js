      ...
      success: function (data) {
        if (data == "ok") {
          ...
          gtag("event", "conversion", {
            send_to: "AW-711350208/iDSpCPG10vcCEMCvmdMC",
          });
          $("#call_tel").val("");
          $(".callback_form").toggleClass("closed");
          ...
          alert(
            "Ошибка при отправлении. Возможно Вы неправильно заполнили поля."
          );
        }
      },
    });
...
function SendComagic(name, tel, type, txt) {
  try {
    if (txt == "") {
      Comagic.push([
        "addOfflineRequest",
        { name: name, phone: tel, message: type },
      ]);
    } else {
      Comagic.push([
        "addOfflineRequest",
        { name: name, phone: tel, email: txt, message: type },
      ]);
    }
  } catch (err) {
    return false;
